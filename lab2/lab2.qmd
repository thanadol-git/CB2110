...
## Q4. 
**Pick one protein that is comprised of 10 detectable peptides. Visulize the peptide intensity. Do we see every peptide? Does every peptide have the same intensity in each sample for each  peptide. If not, why?**

```{r}

## Select a protein with 10 peptides
prot_10 <- ms_result %>% 
  select(ProteinName, PeptideSequence) %>%
  distinct() %>%
  count(ProteinName) %>%
...
ms_result %>% 
  filter(ProteinName == prot_10$ProteinName) %>%  
