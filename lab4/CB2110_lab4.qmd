...
Questions are distributed throughout this notebook. You can view where the questions are by pressing on the "table of contents" icon right below the Google Collab icon, which is situatied on the top left on this page. Good luck with all the questions! Reach out to the TA's if you have any questions regarding this lab.
...
* Integrating different omics data - Focusing on an unsupervised model such as Multi-Omics Factor Analys (MOFA)
...
Pick 2 mutations from any selected feature (g.e., IGHV), use the function `plot_factor()` to show their distribution. For each gene, include the respective protein concentration as given in [the disease section](https://www.proteinatlas.org/humanproteome/disease) of [the Human Protein Atlas (HPA)](https://www.proteinatlas.org/).
Here's an example using FLT3 (don't select FLT3): https://www.proteinatlas.org/ENSG00000122025-FLT3/disease
...
**TIP**: Gene names can be found by picking different factors using the function `plot_top_weights()`
...
Working with latent factors and weights can be daunting as they can seem ambigious. To perhaps make more sense of the meaning of latent factor and weights, this data have been extracted into dataframes from the trained MOFA object as `factors_df` and `weights_df`.
...
Pick a latent factor and run the code in this notebook to describe as much information you could get from that factor, i.e., association to outcome, relation to other factors, feature weights, top features, enrichment analysis, etc.
